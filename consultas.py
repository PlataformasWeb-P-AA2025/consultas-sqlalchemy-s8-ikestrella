...
entregas = session.query(Entrega, Tarea, Estudiante, Instructor, Departamento).join(Tarea).join(Estudiante).join(Curso).join(Instructor).join(Departamento).filter(Departamento.nombre=='Arte')


# Se muestra lo obtenido de la consulta
for e in entregas:
    print(f"Nombre Tarea: {e[1].titulo} - Nombre Estudiante: {e[2].nombre} - Calificacion: {e[0].calificacion} - Nombre Instructor: {e[3].nombre} - Departamento: {e[4].nombre}")
    print()